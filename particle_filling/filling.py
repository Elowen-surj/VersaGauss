    ...
    for i, j, k in grid:
        if grid[i, j, k] == 0:
            ...
            if collision_hit:
                ...
                # print(hit_times)
                if ti.math.mod(hit_times, 2) == 1 or grid[i, j, k] >= max_particles_per_cell:
                    diff = max_particles_per_cell - grid[i, j, k]
                    grid[i, j, k] = max_particles_per_cell
                    tmp_start_idx = ti.atomic_add(new_start_idx, diff)
                    for index in range(tmp_start_idx, tmp_start_idx + diff):
...
def particle_reducing(
    ...
    grid_dx: float,
    start_idx: int,
    opacity_quene: ti.types.ndarray(dtype=ti.f32, ndim=1),
    index_quene: ti.types.ndarray(dtype = ti.i32, ndim=1)
) -> int:
    new_start_idx = start_idx
    internal_scale = scale ** 2 if scale < 0.5 else scale
    for i, j, k in grid:
        max_particles_per_cell_in = ti.max(1, ti.round(grid[i, j, k] * (scale ** 3), dtype = int))
        # print(max_particles_per_cell_in)
        max_particles_per_cell_surface = ti.max(1, ti.round(grid[i, j, k] * (scale ** 2), dtype = int))
        if grid[i, j, k] > max_particles_per_cell_surface:
            collision_hit = True
            for dir_type in ti.static(range(6)):
                hit_test = collision_search(
                    grid=grid,
            ...
            if collision_hit:
                ...
                if ti.math.mod(hit_times, 2) == 1:
                    if grid[i, j, k] > max_particles_per_cell_in:
                    #     # ti_new_cov = ti.Vector.field(n=6, dtype=float, shape=max_particles_per_cell_surface)
                    #     # for mi in ti_new_cov:
                    #     #     for mk in range(6):
                    #     #         ti_new_cov[mi][mk] = 0
                        # for tindex in range(max_particles_per_cell_surface):
                        #     opacity_quene[tindex] = float(1e10)
                        #     index_quene[tindex] = -1
                    #     if max_particles_per_cell_in == 1:
                    #         max_opacity = 0.0
                    #         max_index = -1
                    #         for pi in range(pos.shape[0]):
                    #             x = pos[pi][0]
                    #             y = pos[pi][1]
                    #             z = pos[pi][2]
                    #             pos_i = ti.floor(x / grid_dx, dtype=int)
                    #             pos_j = ti.floor(y / grid_dx, dtype=int)
                    #             pos_k = ti.floor(z / grid_dx, dtype=int)
                    #             if pos_i == i and pos_j == j and pos_k == k:
                    #                 if max_opacity < opacity[pi]:
                    #                     max_opacity = opacity[pi]
                    #                     max_index = pi
                    #         ti_new_cov[new_start_idx] = cov[max_index]
                    #         ti_new_pos[new_start_idx] = pos[max_index]
                    #         ti_new_shs[new_start_idx] = shs[max_index]
                    #         ti_new_opacity[new_start_idx] = opacity[max_index]
                    #         ti.atomic_add(new_start_idx, 1)
                        # opacity_quene.fill(float(1e10))
                        # index_quene.fill(-1)
                        # compute_opacity_quene(pos, opacity, max_particles_per_cell_surface, opacity_quene, index_quene, i, j, k, grid_dx)
                    #     else:
                        for pi in range(pos.shape[0]):
                            x = pos[pi][0]
                            y = pos[pi][1]
                            ...
                            pos_k = ti.floor(z / grid_dx, dtype=int)
                            if pos_i == i and pos_j == j and pos_k == k:
                                # cov_trace = cov[pi][0] + cov[pi][3] + cov[pi][5]
                                # min_opacity = float(1e10)
                                # min_index = -1
                                # for tindex in range(max_particles_per_cell_in):
                                #     if opacity_quene[tindex] < min_opacity:
                                #         min_opacity = opacity_quene[tindex]
                                #         min_index = tindex
                                # if opacity[pi] > min_opacity:
                                #     opacity_quene[min_index] = opacity[pi]
                                #     index_quene[min_index] = pi

                                select_p = ti.random()
                                if select_p < (scale ** 3):
                                    ti_new_cov[new_start_idx] = cov[pi]
                                    ti_new_pos[new_start_idx] = pos[pi]
                                    ti_new_shs[new_start_idx] = shs[pi]
                                    ti_new_opacity[new_start_idx] = opacity[pi]
                                    ti.atomic_add(new_start_idx, 1)
                                    # ti_new_shs[new_start_idx] = [0.886226925452758, -1.772453850905516, -1.772453850905516]

                        # for tindex in range(max_particles_per_cell_in):
                        #     ti_new_cov[new_start_idx] = cov[index_quene[tindex]]
                        #     ti_new_pos[new_start_idx] = pos[index_quene[tindex]]
                        #     ti_new_shs[new_start_idx] = shs[index_quene[tindex]]
                        #     ti_new_opacity[new_start_idx] = opacity[index_quene[tindex]]
                        #     ti.atomic_add(new_start_idx, 1)
                                # index_quene[tindex] = -1
                                # opacity_quene[tindex] = float(1e10)
                    else:
                ...
                #     for mk in range(6):
                #         ti_new_cov[mi][mk] = 0
                # opacity_quene = ti.filed(dtype=float, shape=max_particles_per_cell_in)
                # opacity_quene.fill(0.0)
                # index_quene = ti.filed(dtype=int, shape=max_particles_per_cell_in)
                # index_quene.fill(0)
                # opacity_quene.fill(float(1e10))
                # index_quene.fill(-1)
                # for tindex in range(max_particles_per_cell_surface):
                ...
                #     index_quene[tindex] = -1
                # ifuuu = False
                # if max_particles_per_cell_surface == 1:
                #     max_opacity = 0.0
                #     max_index = -1
                #     for pi in range(pos.shape[0]):
                #         x = pos[pi][0]
                ...
                for pi in range(pos.shape[0]):
                    ...
                    if pos_i == i and pos_j == j and pos_k == k:
                        select_p = ti.random()
                        if select_p < internal_scale:
                            ti_new_cov[new_start_idx] = cov[pi] * scale ** (2 / 3)
                            ti_new_pos[new_start_idx] = pos[pi]
                            ti_new_shs[new_start_idx] = shs[pi]
                            ti_new_opacity[new_start_idx] = opacity[pi]
                            ti.atomic_add(new_start_idx, 1)
                            # ti_new_shs[new_start_idx] = [-0.96931,0.0,0.9554634]
                        ...
                        # ifuuu = True
                # if max_particles_per_cell_surface > 1:
                #     print('uuuuuuu', index_quene[0], opacity_quene[0], ifuuu)
                # for tindex in range(max_particles_per_cell_surface):
                #     ti_new_cov[new_start_idx] = cov[index_quene[tindex]]
                #     ti_new_pos[new_start_idx] = pos[index_quene[tindex]]
                #     ti_new_shs[new_start_idx] = shs[index_quene[tindex]]
...
# @ti.func
# def compute_opacity_quene(pos: ti.template(), opacity: ti.template(), num: int, opacity_quene: ti.types.ndarray(dtype=ti.f32, ndim=1), index_quene: ti.types.ndarray(dtype = ti.i32, ndim=1), i:int, j:int, k:int, grid_dx: float):
#     for pi in range(pos.shape[0]):
#         x = pos[pi][0]
#         y = pos[pi][1]
#         z = pos[pi][2]
...
#             max_opacity = 0.0
#             max_index = -1
#             for tindex in range(num):
#                 if max_opacity < opacity[pi]:
#                     max_opacity = opacity[pi]
#                     max_index = tindex
#             if opacity[pi] < max_opacity:
#                 opacity_quene[max_index] = opacity[pi]
#                 index_quene[max_index] = pi
...
def reduce_particles(
    ...
    grid_n = int((pos.shape[0] * scale ** 3) ** (1 / 3))
    pos_clone = pos.clone()
    max_diff = 0.0
    for i in range(3):
        max_diff = ti.max(max_diff, pos_clone[:, i].max().item() - pos_clone[:, i].min().item())
    grid_dx = max_diff / grid_n
    new_origin = torch.tensor([torch.min(pos_clone[:, 0]), torch.min(pos_clone[:, 1]), torch.min(pos_clone[:, 2])]).cuda()
    pos = pos - new_origin
    ...
    # new_shs = torch.mean(shs, dim=0).repeat(pos.shape[0], 1).cuda()
    ti_new_pos = ti.Vector.field(n=3, dtype=float, shape=pos.shape[0])
    ti_new_shs = ti.Vector.field(n=shs.shape[1], dtype=float, shape=pos.shape[0])
    ti_new_opacity = ti.field(dtype=float, shape=pos.shape[0])
    ti_new_cov = ti.Vector.field(n=6, dtype=float, shape=pos.shape[0])
    opacity_quene = ti.ndarray(dtype=ti.f32, shape=(int(pos.shape[0] * scale),))
    index_quene = ti.ndarray(dtype=ti.i32, shape=(int(pos.shape[0] * scale),))
    opacity_quene.fill(float(0))
    index_quene.fill(-1)
    # ti_new_shs.from_torch(new_shs)

    # breakpoint()
    fill_num = particle_reducing(ti_pos, ti_cov, ti_shs, ti_opacity, grid, grid_density, ti_new_opacity, ti_new_cov, ti_new_pos, ti_new_shs, threshold, scale, ray_cast_dir, grid_dx, fill_num, opacity_quene, index_quene)
    # breakpoint()
    print("After reduce particles: ", fill_num)

    pos_tensor = ti_new_pos.to_torch()[:fill_num].cuda()
    pos_tensor = pos_tensor + new_origin

    shs_tensor = ti_new_shs.to_torch()[:fill_num].cuda()
    opacity_tensor = ti_new_opacity.to_torch()[:fill_num].cuda()
    cov_tensor = ti_new_cov.to_torch()[:fill_num].cuda()
    # breakpoint()

    shs_tensor = shs_tensor.view(shs_tensor.shape[0], -1, 3)
    opacity_tensor = opacity_tensor.reshape(-1, 1)
    # breakpoint()

    return pos_tensor, shs_tensor, opacity_tensor, cov_tensor
